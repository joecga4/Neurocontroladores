% Algoritmo de retroprogación de errores
...
yb1 = 0.075*yb1;
yb1 = yb1 + 2*0.1*randn(N,1);
yb2 = a3*x2.^3 + a2*x2.^2 + a1*x1.^1 + a0;
yb2 = 0.075*yb2;
yb2 = yb2 + 1.5*0.1*randn(N,1);
yb = [ yb1  yb2 ];
...
u = 1*0.35*randn(ne,nm);
v = 1*0.35*randn(nm,pq);
w = 1*0.35*randn(pq,ns);
%load pesos;
...
for k = 1:N   
  in = (x(k,:))';
  m = u'*in;
  n = 2.0./(1+exp(-m./1)) - 1;   % Sigmoidea Tipo 2
  p = v'*n;
