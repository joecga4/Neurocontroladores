...
etapa = 3;   % <-- EDITAR: 1 (cerca) -> 2 (medio) -> 3 (cobertura completa)
...
load redcarro11_etapa2;   % VALIDACIÓN PARCIAL: arranca del pesos de la etapa 2
...
while( (erreltotal > errormax) && (cont < contmax) )
    ...
    if( mod(cont,200) == 0 )
        save redcarro11_etapa3 nm v w c a;
        fprintf('  [checkpoint] redcarro11_etapa3.mat guardado en iteracion %d\n', cont);
    end
    cont = cont + 1;
...
if( guardar == 1 )
    save redcarro11_etapa3 nm v w c a;
    disp('Pesos guardados en redcarro11_etapa3.mat');
end
