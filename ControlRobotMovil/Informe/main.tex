...
El controlador se entrena por \textbf{retropropagación dinámica (DBP, \emph{Dynamic Back
Propagation})}: el gradiente del
...
Los scripts base son \code{DynamicBPCarro.m} (entrenamiento del carro por DBP) y
\code{DynamicBPCarroValida.m} (validación). Para el trailer no existía script: se
\textbf{implementó desde cero} el modelo cinemático articulado y su entrenador DBP.
...
\textbf{entrenamiento por etapas} (de cerca a lejos) con \textbf{DBP truncado} (ventana \code{Ttrunc})
y \textbf{recorte de gradiente} (\code{gmax}) para estabilizar el horizonte largo
($ndata=6000$). El núcleo DBP vectoriza la recursión de sensibilidades con el jacobiano
del lazo cerrado \code{jacob\_t = dxdu*dudx + jacob}.

\subsection{Generación de las trayectorias}
La misma red se usa con referencias $(x^*,\phi^*)$ que se recalculan en cada paso según
la geometría de la trayectoria deseada. En todos los casos la red recibe el error
$[\,x-x^*,\ \phi-\phi^*\,]$ y, para la circular, se suma un término de dirección
$\delta^*$.

\paragraph{Estacionamiento vertical.} La meta es fija: $x^*=0$, $\phi^*=\pi/2$
(regulación directa, sin $\delta^*$).

\paragraph{Trayectoria oblicua (recta a $\theta$).} La recta deseada pasa por el origen,
$y=\tan\theta\,x$. La posición deseada $x^*$ es la abscisa del \emph{pie de la
perpendicular} del punto actual $(x,y)$ sobre esa recta, y la orientación deseada es la
pendiente de la recta:
\[
  x^{*}=\big(x\cos\theta+y\sin\theta\big)\cos\theta, \qquad
  \phi^{*}=\theta, \qquad \delta^{*}=0 .
\]
Para $\theta=45^\circ$ se reduce a $x^{*}=(x+y)/2$ y $\phi^{*}=\pi/4$; para
$\theta=60^\circ$, $x^{*}=(x+\sqrt{3}\,y)/4$ y $\phi^{*}=\pi/3$.

\paragraph{Trayectoria circular (radio $R$, centro $(x_c,y_c)$).} Se sigue la
\emph{recta tangente local} al círculo. Con el ángulo del punto actual respecto al centro
$\alpha=\operatorname{atan2}(y-y_c,\;x-x_c)$, el punto más cercano del círculo es
$P=(x_c+R\cos\alpha,\;y_c+R\sin\alpha)$ y el rumbo tangente (sentido horario)
$\phi^{*}=\alpha-\pi/2$. La posición deseada $x^{*}$ es la abscisa del pie de la
perpendicular de $(x,y)$ sobre esa tangente (dirección
$\mathbf{d}=(\cos\phi^{*},\sin\phi^{*})$), y se añade una dirección de curvatura constante:
\[
  x^{*}=P_x+\big[(\,(x,y)-P)\cdot\mathbf{d}\,\big]\cos\phi^{*}, \qquad
  \phi^{*}=\alpha-\tfrac{\pi}{2}, \qquad
  \delta^{*}=\operatorname{atan}\!\big(L/R\big) .
\]
...
\]
Se implementó (\code{train\_trailer.m}) el entrenador DBP sobre el estado de control
...
En la práctica, el DBP desde cero \emph{no} convergió a una política de estacionamiento
...
\label{sec:car}
El entrenamiento DBP converge: en una etapa del entrenamiento por etapas el costo cae al $\sim5\%$ de
...
\caption{Convergencia del entrenamiento DBP del carro (costo relativo $J/J_0$).}
\label{fig:cartrain}
\end{figure}
...
\begin{enumerate}
  ...
  carro, entrenado por DBP solo para \emph{regulación} ($x^*=0,\ \phi^*=\pi/2$) pero
  ...
  \item \textbf{El entrenamiento DBP requiere cuidados.} Por el horizonte largo, fueron
  necesarios el \emph{entrenamiento por etapas} incremental, la \emph{normalización} de entradas, el
  \emph{DBP truncado} y el \emph{recorte de gradiente}; con ellos el costo converge de
  forma estable.
  ...
  articulación con el tractor), el DBP desde cero no convergió a una política
  satisfactoria. Se implementaron su modelo articulado de cuatro estados y el entrenador
  DBP, se verificó que el modelo es controlable con una ley estabilizadora, y la red del
  ...
  DBP plenamente convergente para el trailer queda como trabajo futuro (entrenamiento por etapas más
...
(entrenamiento DBP del trailer) y \code{trailer\_experiments.m} (sus trayectorias). Las
